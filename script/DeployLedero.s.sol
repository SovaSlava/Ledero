...
contract DeployLedero is Script {
    function run() external {
        ...
        BalancerV3Adapter balancerAdapter = new BalancerV3Adapter{salt: flashSalt}(vm.envAddress("BALANCER_VAULT"));
        ...
        OneInchAdapter swapAdapter = new OneInchAdapter{salt: swapSalt}(vm.envAddress("ONE_INCH_ROUTER"));
    ...
    function getVanitySalt(bytes memory initCode, uint256 prefix) internal returns (bytes32) {
        ...
        // solhint-disable-next-line, foundry-cheatcodes
        // forge-lint: disable-next-line(unsafe-cheatcode)
        bytes memory rawData = vm.ffi(inputs);

        bytes memory res = abi.decode(rawData, (bytes));
        bytes32 salt;
        require(res.length >= 32, "Invalid vanity salt length");
        assembly ("memory-safe") {
            salt := mload(add(res, 0x20))
        }
        return salt;
    }
}
