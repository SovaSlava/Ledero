...
abstract contract FfiHelper is Test {
        ...
        returns (bytes memory swapData, uint256 expectedAmount)
    {
        ...
        inputs[0] = "npx";
        inputs[1] = "tsx";
        inputs[2] = "script/get_1inch_data.ts";
        inputs[3] = vm.toString(fromToken);
        inputs[4] = vm.toString(toToken);
        inputs[5] = vm.toString(amount);
        inputs[6] = vm.toString(fromAddress);

        (bool success, bytes memory rawData) = address(vm).call(abi.encodeWithSignature("ffi(string[])", inputs));
        require(success, "1inch FFI execution failed");
        bytes memory result = abi.decode(rawData, (bytes));
        ...
        string memory toAmountStr = abi.decode(toAmountBytes, (string));
        expectedAmount = vm.parseUint(toAmountStr);
