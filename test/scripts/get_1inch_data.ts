...
async function get1inchSwapData(
  ...
  const params = {
    ...
    protocols: "UNISWAP_V3,SUSHISWAP_V3,PANCAKESWAP_V3,CURVE,CURVE_V2,BALANCER_V2,AAVE_V3"
  };
...
async function main() {
  ...
  if (process.argv.length < 6) {
    console.log(
      JSON.stringify({
        error: "Usage: npx ts-node get_1inch_data.ts <fromToken> <toToken> <amount> <fromAddress>",
      })
    );
    process.exit(1);
  }
  ...
  const result = await get1inchSwapData(fromToken, toToken, amount, fromAddress);
